...
public class Detective extends Role implements Power {

    private boolean canSee = false;

    public Detective(LGGame game) {
        super(game);
    }

    public static RoleIdentity getStaticRoleIdentity() {
        return new RoleIdentity("Detective", WinType.VILLAGE, RoleType.VILLAGER);
    }
    ...
    protected void newEpisode(LGGame game, UHCPlayer lgp) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (i==1) {
                    lgp.sendMessage(getPrefix() + " " + getPrefixPrivé() + ChatColor.GOLD + " Tu peut comparer deux joueurs en faisant /lg role see <player> <player>");
                }
                if (!canSee) {
                    cancel();
                }
